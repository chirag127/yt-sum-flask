...
def gensim_summarize(text_content, percent):
    ...
    return summarize(
        text_content, ratio=(int(percent) / 100), split=False
    ).replace("\n", " ")
...
def spacy_summarize(text_content, percent):
    ...
    for word in nlp_object:
        if (
            ...
            and word.text.lower() not in punctuation_items
        ):
    ...
    sentence_token = list(nlp_object.sents)
...
def nltk_summarize(text_content, percent):
    ...
    for word in tokens:
        if (
            ...
            and word.lower() not in punctuation_items
        ):
...
def sumy_lsa_summarize(text_content, percent):
    ...
    return "".join(
        ...
        for sentence in summarizer(
            parser.document, sentences_count=select_length
        )
    )
